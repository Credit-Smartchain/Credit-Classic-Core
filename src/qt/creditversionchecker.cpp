...
#define paternVersion "credit-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

CreditVersionChecker::CreditVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

CreditVersionChecker::~CreditVersionChecker()
{

}

bool CreditVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> CreditVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(CREDIT_RELEASES)));
    QEventLoop event;
...
Version CreditVersionChecker::getMaxReleaseVersion()
{
