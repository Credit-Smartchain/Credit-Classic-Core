#include <qt/crctoken.h>
#include <qt/forms/ui_crctoken.h>
...
CRCToken::CRCToken(const PlatformStyle *platformStyle, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CRCToken),
    m_model(0),
    m_clientModel(0),
    m_tokenTransactionView(0)
{
    ...
    connect(m_tokenList, &TokenListWidget::sendToken, this, &CRCToken::on_sendToken);
    connect(m_tokenList, &TokenListWidget::receiveToken, this, &CRCToken::on_receiveToken);
    connect(m_tokenList, &TokenListWidget::addToken, this, &CRCToken::on_addToken);
    ...
    connect(copyTokenAddressAction, &QAction::triggered, this, &CRCToken::copyTokenAddress);
    connect(copyTokenBalanceAction, &QAction::triggered, this, &CRCToken::copyTokenBalance);
    connect(copyTokenNameAction, &QAction::triggered, this, &CRCToken::copyTokenName);
    connect(copySenderAction, &QAction::triggered, this, &CRCToken::copySenderAddress);
    connect(removeTokenAction, &QAction::triggered, this, &CRCToken::removeToken);

    connect(m_tokenList, &TokenListWidget::customContextMenuRequested, this, &CRCToken::contextualMenu);
}
...
void CRCToken::setModel(WalletModel *_model)
{
    m_model = _model;
    ...
    {
        // Set current token
        connect(m_tokenList->tokenModel(), &QAbstractItemModel::dataChanged, this, &CRCToken::on_dataChanged);
        connect(m_tokenList->tokenModel(), &QAbstractItemModel::rowsInserted, this, &CRCToken::on_rowsInserted);
...
void CRCToken::setClientModel(ClientModel *_clientModel)
{
...
void CRCToken::on_goToSendTokenPage()
{
    m_sendTokenPage->show();
}

void CRCToken::on_goToReceiveTokenPage()
{
    m_receiveTokenPage->show();
}

void CRCToken::on_goToAddTokenPage()
{
    m_addTokenPage->show();
}

void CRCToken::on_currentTokenChanged(QModelIndex index)
{
...
void CRCToken::on_dataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> &roles)
{
...
void CRCToken::on_currentChanged(QModelIndex current, QModelIndex previous)
{
    Q_UNUSED(previous);
...
void CRCToken::on_rowsInserted(QModelIndex index, int first, int last)
{
    Q_UNUSED(index);
    Q_UNUSED(first);
    Q_UNUSED(last);
...
void CRCToken::contextualMenu(const QPoint &point)
{
...
void CRCToken::copyTokenAddress()
{
...
void CRCToken::copyTokenBalance()
{
    if(indexMenu.isValid())
    {
...
void CRCToken::copySenderAddress()
{
    if(indexMenu.isValid())
    {
...
void CRCToken::on_sendToken(const QModelIndex &index)
{
    on_currentTokenChanged(index);
    on_goToSendTokenPage();
}

void CRCToken::on_receiveToken(const QModelIndex &index)
{
    on_currentTokenChanged(index);
    on_goToReceiveTokenPage();
}

void CRCToken::on_addToken()
{
    on_goToAddTokenPage();
}
