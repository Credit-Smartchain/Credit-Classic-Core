...
#include <qt/creditversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    creditVersionChecker = new CreditVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    CRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/crctoken"), tr("&CRC Tokens"), this);
    CRCTokenAction->setStatusTip(tr("CRC Tokens (send, receive or add Tokens in list)"));
    CRCTokenAction->setToolTip(CRCTokenAction->statusTip());
    CRCTokenAction->setCheckable(true);
    CRCTokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
    tabGroup->addAction(CRCTokenAction);
    ...
    connect(CRCTokenAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(CRCTokenAction, SIGNAL(triggered()), this, SLOT(gotoTokenPage()));
    ...
    openAction->setStatusTip(tr("Open a credit: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Credit command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->addAction(CRCTokenAction);
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && creditVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(CREDIT_RELEASES, CREDIT_RELEASES);
            QString message(tr("New version of Credit wallet is available on the Credit source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    CRCTokenAction->setEnabled(enabled);
...
{
    CRCTokenAction->setChecked(true);
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Credit network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
