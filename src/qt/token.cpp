...
bool ToHash160(const std::string& strCreditAddress, std::string& strHash160)
{
...
bool ToCreditAddress(const std::string& strHash160, std::string& strCreditAddress)
{
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToCreditAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToCreditAddress(tokenEvent.receiver, tokenEvent.receiver);
