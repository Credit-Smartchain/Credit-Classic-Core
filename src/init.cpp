 ...
 */
static const char* BITCOIN_PID_FILENAME = "creditd.pid";
...
void Shutdown(NodeContext& node)
{
    ...
    util::ThreadRename("credit-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/The-Terra-Foundation/Credit-Core>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateCredit");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path creditStateDir = GetDataDir() / "stateCredit";
                bool fStatus = fs::exists(creditStateDir);
                const std::string dirCredit(creditStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsCreditstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<CreditState>(new CreditState(dev::u256(0), CreditState::openDB(dirCredit, hashDB, dev::WithExisting::Trust), dirCredit, existsCreditstate));
                dev::eth::ChainParams cp(chainparams.EVMGenesisInfo());
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(creditStateDir.string()));
                if (fReset) {
            ...
            try {
                ...
                globalSealEngine->setCreditSchedule(creditDGP.getGasSchedule(::ChainActive().Height() + (::ChainActive().Height()+1 >= chainparams.GetConsensus().CIP7Height ? 0 : 1) ));
