...
CreditDelegation& GetCreditDelegation()
{
    static CreditDelegation creditDelegation;
    return creditDelegation;
}
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, BlockValidationState& state, const CTransaction& tx, unsigned int nBits, uint32_t nTimeBlock, const std::vector<unsigned char>& vchPoD,  const COutPoint& headerPrevout, uint256& hashProofOfStake, uint256& targetProofOfStake, CCoinsViewCache& view)
{
    ...
    if (nHeight >= nOfflineStakeHeight && !Params().GetConsensus().delegationsAddress.IsNull())
    {
        ...
        CreditDelegation& creditDelegation = GetCreditDelegation();
        if(!creditDelegation.ExistDelegationContract())
        ...
        bool verifiedDelegation = creditDelegation.VerifyDelegation(address, delegation);
...
bool CheckKernel(CBlockIndex* pindexPrev, unsigned int nBits, uint32_t nTimeBlock, const COutPoint& prevout, CCoinsViewCache& view, const std::map<COutPoint, CStakeCache>& cache)
{
    ...
    if(it == cache.end()) {
        ...
        const CBlockIndex* pforkBase = ChainActive().FindFork(pindexPrev);
        LogPrint(BCLog::VALIDATION, "CheckKernel() : pforkBase->nHeight: %d\n", pforkBase->nHeight);
        CBlockIndex* pindex = ChainActive().Tip();
        LogPrint(BCLog::VALIDATION, "CheckKernel() : pindex->nHeight: %d\n", pindex->nHeight);
        if(coinPrev.IsSpent() && pindex->nHeight > pforkBase->nHeight){
...
bool GetDelegationFeeFromContract(const uint160& address, uint8_t& fee)
{
    Delegation delegation;
    CreditDelegation& creditDelegation = GetCreditDelegation();
    bool ret = creditDelegation.GetDelegation(address, delegation);
    if(ret) ret &= creditDelegation.VerifyDelegation(address, delegation);
    if(ret)
    {
        fee = delegation.fee;
    }
