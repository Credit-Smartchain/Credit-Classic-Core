...
static void ProcessBlockAvailability(NodeId nodeid) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    CNodeState *state = State(nodeid);
    assert(state != nullptr);
    LogPrint(BCLog::NET, "%s: started =%d\n", __func__, nodeid);

    if (!state->hashLastUnknownBlock.IsNull()) {
        const CBlockIndex* pindex = LookupBlockIndex(state->hashLastUnknownBlock);
        LogPrint(BCLog::NET, "%s: pindex == nullptr = %d =%d\n", __func__, (pindex == nullptr), nodeid);
...
static void UpdateBlockAvailability(NodeId nodeid, const uint256 &hash) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    LogPrint(BCLog::NET, "%s: hash = %s =%d\n", __func__, hash.ToString(), nodeid);
...
static void FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<const CBlockIndex*>& vBlocks, NodeId& nodeStaller, const Consensus::Params& consensusParams) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (state->pindexBestKnownBlock == nullptr || state->pindexBestKnownBlock->nChainWork <= ::ChainActive().Tip()->nChainWork || state->pindexBestKnownBlock->nChainWork < nMinimumChainWork) {
        LogPrint(BCLog::NET, "%s: state->pindexBestKnownBlock == nullptr = %d =%d\n", __func__, (state->pindexBestKnownBlock == nullptr), nodeid);
    ...
    if (state->pindexLastCommonBlock == state->pindexBestKnownBlock) {
        LogPrint(BCLog::NET, "%s: LastCommonBlock equals BestKnownBlock =%d\n", __func__, nodeid);
        return;
    }
    ...
    while (pindexWalk->nHeight < nMaxHeight) {
        ...
        for (const CBlockIndex* pindex : vToFetch) {
            if (!pindex->IsValid(BLOCK_VALID_TREE)) {
                LogPrint(BCLog::NET, "%s: Block isn't valid =%d\n", __func__, nodeid);
                // We consider the chain that this peer is on invalid.
                return;
            }
            if (!State(nodeid)->fHaveWitness && IsWitnessEnabled(pindex->pprev, consensusParams)) {
                LogPrint(BCLog::NET, "%s: Witness Problem =%d\n", __func__, nodeid);
            ...
            if (pindex->nStatus & BLOCK_HAVE_DATA || ::ChainActive().Contains(pindex)) {
                ...
                if (pindex->nHeight > nWindowEnd) {
                    ...
                    LogPrint(BCLog::NET, "%s: End of Window =%d\n", __func__, nodeid);
                    return;
                }
                vBlocks.push_back(pindex);
                if (vBlocks.size() == count) {
                    LogPrint(BCLog::NET, "%s: Block count done =%d\n", __func__, nodeid);
                    return;
                }
...
void PeerLogicValidation::UpdatedBlockTip(const CBlockIndex *pindexNew, const CBlockIndex *pindexFork, bool fInitialDownload) {
    ...
    LogPrint(BCLog::NET, "%s fInitialDownload (%d)\n", __func__, fInitialDownload);
...
bool static ProcessHeadersMessage(CNode* pfrom, CConnman* connman, CTxMemPool& mempool, const std::vector<CBlockHeader>& headers, const CChainParams& chainparams, bool via_compact_block)
{
    ...
    if (nCount == 0) {
        ...
        LogPrint(BCLog::NET, "%s: headers nCount is 0 =%d\n", __func__, pfrom->GetId());
        return true;
    }
...
bool ProcessMessage(CNode* pfrom, const std::string& msg_type, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CTxMemPool& mempool, CConnman* connman, BanMan* banman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        if (::ChainActive().Tip()->nHeight >= chainparams.GetConsensus().CIP7Height && nVersion < MIN_PEER_PROTO_VERSION_AFTER_CIP7) {
        	// disconnect from peers older than this proto version
        	LogPrint(BCLog::NET, "peer=%d using obsolete version after CIP7 hardfork %i; disconnecting\n", pfrom->GetId(), nVersion);
        	pfrom->fDisconnect = true;
        	return false;
        }
    ...
    if (msg_type == NetMsgType::GETHEADERS) {
        ...
        if (::ChainstateActive().IsInitialBlockDownload() && !pfrom->HasPermission(PF_NOBAN)) {
            LogPrint(BCLog::NET, "%s: IsInitialBlockDownload() = %d peer = %d\n", __func__, ::ChainstateActive().IsInitialBlockDownload(), pfrom->GetId());
        ...
        if (locator.IsNull())
        {
            ...
            if (!pindex) {
                LogPrint(BCLog::NET, "%s: pindex==nullptr Lookup = %d peer = %d\n", __func__, 0, pfrom->GetId());
                return true;
            }
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        LogPrint(BCLog::NET, "%s: send messages to peer=%d\n", __func__, pto->GetId());
        ...
        LogPrint(BCLog::NET, "%s: MaybeDiscourageAndDisconnect passed =%d\n", __func__, pto->GetId());
        CNodeState &state = *State(pto->GetId());
        ...
        //
        {
            ...
            std::vector<CBlock> vHeaders;
            LogPrint(BCLog::NET, "state.fPreferHeaders=%d \n", state.fPreferHeaders);
            LogPrint(BCLog::NET, "state.fPreferHeaderAndIDs=%d \n", state.fPreferHeaders);
            LogPrint(BCLog::NET, "pto->vBlockHashesToAnnounce.size()=%d \n", pto->vBlockHashesToAnnounce.size());
        ...
        LogPrint(BCLog::NET, "%s: FindNextBlocksToDownload try =%d\n", __func__, pto->GetId());
        ...
        if (!pto->fClient && ((fFetch && !pto->m_limited_node) || !::ChainstateActive().IsInitialBlockDownload()) && state.nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            ...
            NodeId staller = -1;
            LogPrint(BCLog::NET, "%s: FindNextBlocksToDownload started =%d\n", __func__, pto->GetId());
