...
const char* const BITCOIN_CONF_FILENAME = "credit.conf";
...
std::string ArgsManager::GetArg(const std::string& strArg, const std::string& strDefault) const
{
    ...
    return value.isNull() ? strDefault : value.isFalse() ? "0" :
                                     value.isTrue()      ? "1" :
                                                           value.get_str();
}
...
{
    const util::SettingsValue value = GetSetting(strArg);
    return value.isNull() ? nDefault : value.isFalse() ? 0 :
                                   value.isTrue()      ? 1 :
...
{
    const util::SettingsValue value = GetSetting(strArg);
    return value.isNull() ? fDefault : value.isBool() ? value.get_bool() :
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Credit Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Credit Core Developers";
    }
