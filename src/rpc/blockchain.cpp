...
double GetPoSKernelPS()
{
    ...
    if(pindex)
    {
        dynamicStakeSpacing = pindex->nHeight < consensusParams.CIP9Height;
...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // credit
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // credit

    if(blockindex->IsProofOfStake()){
        result.pushKV("prevoutStakeHash", blockindex->prevoutStake.hash.GetHex()); // credit
        result.pushKV("prevoutStakeVoutN", (int64_t)blockindex->prevoutStake.n); // credit
    }
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // credit
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // credit
...
UniValue transactionReceiptToJSON(const CreditTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
static std::vector<RPCResult> MempoolEntryDescription() { return {
    ...
    RPCResult{RPCResult::Type::NUM, "size", "(DEPRECATED) same as vsize. Only returned if creditd is started with -deprecatedrpc=size\n"
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        CTxDestination creditSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(creditSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&creditSenderAddress);
...
UniValue getdelegationinfoforaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationinfoforaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The credit address string"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
    ...
    CreditDelegation creditDelegation;
    Delegation delegation;
    uint160 address = uint160(*pkhash);
    if(!creditDelegation.GetDelegation(address, delegation)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegation");
    }
    bool verified = creditDelegation.VerifyDelegation(address, delegation);
...
UniValue getdelegationsforstaker(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationsforstaker",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The credit address string for staker"},
                },
               RPCResult{
            RPCResult::Type::ARR, "", "",
                {
    ...
    if(!creditDelegation.FilterDelegationEvents(events, filter)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegations for staker");
    }
    std::map<uint160, Delegation> delegations = creditDelegation.DelegationsFromEvents(events);
                        ...
                        {RPCResult::Type::OBJ, "scriptPubKey", "",
                            {
                                ...
                                {RPCResult::Type::ARR, "addresses", "array of credit addresses",
                                    {{RPCResult::Type::STR, "address", "credit address"}}},
                            }},
