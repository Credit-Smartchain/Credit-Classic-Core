...
std::unique_ptr<CreditState> globalState;
...
bool fAddressIndex = false; // credit
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        uint64_t minGasPrice = creditDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        uint64_t blockGasLimit = creditDGP.getBlockGasLimit(::ChainActive().Tip()->nHeight + 1);
        size_t count = 0;
        ...
        CreditTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractCreditTX resultConverter;
        if(!converter.extractionCreditTransactions(resultConverter)){
        ...
        std::vector<CreditTransaction> creditTransactions = resultConverter.first;
        std::vector<EthTransactionParams> creditETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(CreditTransaction creditTransaction : creditTransactions){
            sumGas += creditTransaction.gas() * creditTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, creditTransaction.gasPrice());
            } else {
                txMinGasPrice = creditTransaction.gasPrice();
            }
            VersionVM v = creditTransaction.getVersion();
            if(v.format!=0)
            ...
            if(creditTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(creditTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(creditTransaction.gas() > UINT32_MAX)
            ...
            gasAllTxs += creditTransaction.gas();
            ...
            if(v.rootVM!=0 && (uint64_t)creditTransaction.gasPrice() < minGasPrice)
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s CREDIT additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // credit
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // credit
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    if (pindex->nHeight >= consensusparams.CIP5Height) {
...
unsigned int GetContractScriptFlags(int nHeight, const Consensus::Params& consensusparams) {
    ...
    // Start support sender address in contract output
    if (nHeight >= consensusparams.CIP5Height) {
        flags |= SCRIPT_OUTPUT_SENDER;
    }
...
bool GetSpentCoinFromBlock(const CBlockIndex* pindex, COutPoint prevout, Coin* coin) {
    ...
    LogPrint(BCLog::VALIDATION, "GetSpentCoinFromBlock() : prevout: %s\n", prevout.ToString());
    ...
    for(size_t j = 1; j < block.vtx.size(); ++j) {
        ...
        for(size_t k = 0; k < tx->vin.size(); ++k) {
            ...
            LogPrint(BCLog::VALIDATION, "tmpprevout: %s\n", tmpprevout.ToString());
...
bool GetSpentCoinFromMainChain(const CBlockIndex* pforkPrev, COutPoint prevoutStake, Coin* coin) {
    ...
    // If it not in any of those blocks, and not in the utxo set, it can't be spendable in the orphan chain.
    {
        ...
        LogPrint(BCLog::VALIDATION, "pindex: %s\n", pindex->ToString());
        LogPrint(BCLog::VALIDATION, "pforkBase: %s\n", pforkBase->ToString());
        if (pindex == pforkBase) {
            return true;
        }
        while(pindex && pindex != pforkBase) {
...
bool CheckOpSender(const CTransaction& tx, const CChainParams& chainparams, int nHeight){
    ...
    if(!(nHeight >= chainparams.GetConsensus().CIP5Height))
        return false;
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount){
    ...
    CreditTransaction callTransaction;
    if(addrContract == dev::Address())
    {
        callTransaction = CreditTransaction(nAmount, 1, dev::u256(gasLimit), opcode, dev::u256(0));
    }
    else
    {
        callTransaction = CreditTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<CreditTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path creditDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(creditDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(creditDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(CreditTransaction& tx : txs){
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{execRes, CreditTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
bool ByteCodeExec::processingResults(ByteCodeExecResult& resultBCE){
    ...
    for(size_t i = 0; i < result.size(); i++){
        ...
        if(result[i].execRes.excepted != dev::eth::TransactionException::None){
        	...
        	if(!(::ChainActive().Height() >= consensusParams.CIP7Height && result[i].execRes.excepted == dev::eth::TransactionException::RevertInstruction)){
        	resultBCE.usedGas += gasUsed;
        	}
        }

        if(result[i].execRes.excepted == dev::eth::TransactionException::None || (::ChainActive().Height() >= consensusParams.CIP7Height && result[i].execRes.excepted == dev::eth::TransactionException::RevertInstruction)){
...
bool CreditTxConverter::extractionCreditTransactions(ExtractCreditTX& credittx){
    ...
    std::vector<CreditTransaction> resultTX;
    ...
    credittx = std::make_pair(resultTX, resultETP);
    return true;
}

bool CreditTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool CreditTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
CreditTransaction CreditTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    CreditTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = CreditTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = CreditTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t CreditTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // credit
    CreditDGP creditDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setCreditSchedule(creditDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().CIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = creditDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().CIP7Height ? 0 : 1));
    uint64_t minGasPrice = creditDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().CIP7Height ? 0 : 1));
    uint64_t blockGasLimit = creditDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().CIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // credit
...
///////////////////////////////////////////////////////////////////////////////////////// credit
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            CreditTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractCreditTX resultConvertCreditTX;
            if(!convert.extractionCreditTransactions(resultConvertCreditTX)){
            ...
            if(!CheckMinGasPrice(resultConvertCreditTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertCreditTX.first, blockGasLimit, pindex->pprev);
            ...
            for(CreditTransaction& qtx : resultConvertCreditTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertCreditTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertCreditTX.first[k].from(),
                        resultConvertCreditTX.first[k].to(),
                        ...
                        resultConvertCreditTX.first[k].getNVout(),
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !Params().MineBlocksOnDemand()) {
        ...
        if(pindex->nMoneySupply > (uint64_t)MAX_MONEY){
...
bool CChainState::ConnectTip(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // credit
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // credit
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // credit
            globalState->setRootUTXO(oldHashUTXORoot); // credit
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = ::ChainActive().Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // credit
        uint32_t sizeBlockDGP = creditDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Credit PoS we need to sync this after every block to ensure txdb is populated for
...
CAmount GetTxGasFee(const CMutableTransaction& _tx)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        CreditTxConverter convert(tx, NULL, NULL, contractflags);
