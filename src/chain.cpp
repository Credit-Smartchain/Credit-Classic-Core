...
const CBlockIndex *CChain::FindFork(const CBlockIndex *pindex) const {
    ...
    LogPrint(BCLog::VALIDATION, "FindFork() : pindex->nHeight: %d\n", pindex->nHeight);
    LogPrint(BCLog::VALIDATION, "FindFork() : Height: %d\n", Height());
    if (pindex->nHeight > Height())
        pindex = pindex->GetAncestor(Height());
    
    if (pindex) {
        LogPrint(BCLog::VALIDATION, "FindFork() : pindex->nHeight: %d As Ancestor\n", pindex->nHeight);
        // int coinbaseMaturity = Params().GetConsensus().CoinbaseMaturity(Height());
        // if (Height() > coinbaseMaturity &&Height() <= pindex->nHeight) {
        //     pindex = pindex->GetAncestor(Height() - coinbaseMaturity);
        // }
    }
