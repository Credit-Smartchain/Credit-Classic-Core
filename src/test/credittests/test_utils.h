...
inline void initState(){
    ...
    const std::string dirCredit = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<CreditState>(new CreditState(dev::u256(0), CreditState::openDB(dirCredit, hashDB, dev::WithExisting::Trust), dirCredit + "/creditDB", dev::eth::BaseState::Empty));
...
inline dev::Address createCreditAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline CreditTransaction createCreditTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    CreditTransaction txEth;
    if(recipient == dev::Address()){
        txEth = CreditTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = CreditTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<CreditTransaction> txs){
