...
//    initState();
    ...
    txs.push_back(createCreditTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    txs.push_back(createCreditTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    executeBC(txs);

    // Call upgrade to
    dev::Address proxy = createCreditAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<CreditTransaction> txsCall;
    txsCall.push_back(createCreditTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    executeBC(txsCall);

    // Call mint
    std::vector<CreditTransaction> txsMint;
    txsMint.push_back(createCreditTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    auto result = executeBC(txsMint);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);

    // Call balance of
    std::vector<CreditTransaction> txsbalance;
    txsbalance.push_back(createCreditTransaction(ParseHex("70a082310000000000000000000000000101010101010101010101010101010101010101"), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
//    initState();
    ...
    // Create contract
    std::vector<CreditTransaction> txs;
    txs.push_back(createCreditTransaction(CODE[4], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    executeBC(txs);

    // Call is it constantinople
    dev::Address proxy = createCreditAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<CreditTransaction> txIsItConstantinople;
    txIsItConstantinople.push_back(createCreditTransaction(CODE[5], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
