...
void checkResult(bool isCreation, std::vector<CreditTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    CreditTxConverter converter(transaction, NULL);
    ExtractCreditTX creditTx;
    BOOST_CHECK(converter.extractionCreditTransactions(creditTx));
    std::vector<CreditTransaction> result = creditTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(credittxconverter_tests, TestingSetup)
