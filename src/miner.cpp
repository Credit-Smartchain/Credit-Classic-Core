...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setCreditSchedule(creditDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = creditDGP.getBlockSize(nHeight);
    minGasPrice = creditDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = creditDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    CreditTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractCreditTX resultConverter;
    if(!convert.extractionCreditTransactions(resultConverter)){
    ...
    for(CreditTransaction creditTransaction : creditTransactions){
        ...
        if(bceResult.usedGas + creditTransaction.gas() > softBlockGasLimit){
        ...
        if(creditTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, creditTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= checkpointSpan)
        {
            ...
            creditDelegations.FilterDelegationEvents(events, *this);
            delegations_staker = creditDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                creditDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                creditDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            creditDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            creditDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    CreditDelegation creditDelegations;
    int32_t cacheHeight;
    ...
    void Update(interfaces::Chain::Lock& locked_chain, int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= checkpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                creditDelegations.FilterDelegationEvents(events, *this);
                pwallet->m_my_delegations = creditDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    creditDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                    creditDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                creditDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(creditDelegations.GetDelegation(address, delegation) && CreditDelegation::VerifyDelegation(address, delegation))
                    {
...
public:
...

    {
        ...
        std::string threadName = "creditstake";
...
public:
    ...
    void Run()
    {
        ...
        while (Next()) {
            ...
            LogPrint(BCLog::COINSTAKE, "StakeMiner(): IsReady Passed \n");

            // Cache mining data
            if(!CacheData()) continue;

            LogPrint(BCLog::COINSTAKE, "StakeMiner(): CacheData Passed \n");
            ...
            if(HaveCoinsForStake())
            {
                ...
                for(uint32_t blockTime = d->beginningTime; blockTime < d->endingTime; blockTime += d->stakeTimestampMask+1)
                {
                    ...
                    LogPrint(BCLog::COINSTAKE, "StakeMiner(): CanCreateBlock() = %d\n", CanCreateBlock(blockTime));
...
void StakeCredits(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
