...
public:
    ...
    bool isUnspentAddress(const std::string &creditAddress) override
    {
        auto locked_chain = m_wallet->chain().lock();
        LOCK(m_wallet->cs_wallet);
        ...
        for (const COutput& out : vecOutputs)
        {
            ...
            if(fValidAddress && EncodeDestination(address) == creditAddress && out.tx->tx->vout[out.i].nValue)
            {
                return true;
            }
        }
    ...
    bool tryGetAvailableAddresses(std::vector<std::string> &spendableAddresses, std::vector<std::string> &allAddresses, bool &includeZeroValue) override
    {
        ...
        includeZeroValue = num_blocks >= Params().GetConsensus().CIP5Height;
    ...
    DelegationInfo getDelegationContract(const std::string &sHash, bool& validated, bool& contractRet) override
    {
        ...
        if (mi != m_wallet->mapDelegation.end()) {
            ...
            if(keyID)
            {
                ...
                contractRet = m_creditDelegation.ExistDelegationContract() ? m_creditDelegation.GetDelegation(address, delegation) : false;
                if(contractRet)
                {
                    validated = m_creditDelegation.VerifyDelegation(address, delegation);
    ...
    DelegationDetails getDelegationDetails(const std::string &sAddress) override
    {
        ...
        {
            uint160 address(*keyID);
            details.c_contract_return = m_creditDelegation.ExistDelegationContract() ? m_creditDelegation.GetDelegation(address, delegation) : false;
            if(details.c_contract_return)
            {
                details.c_entry_exist = m_creditDelegation.VerifyDelegation(address, delegation);
    ...
    CreditDelegation m_creditDelegation;
};
