...
CreditState::CreditState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = CreditState::openDB(_path + "/creditDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

CreditState::CreditState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute CreditState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, CreditTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createCreditAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (!e.execute()){
            e.go(onOp);
            if(ChainActive().Height() >= consensusParams.CIP7Height){
        ...
        if (_p == Permanence::Reverted){
            ...
            credit::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
    ...
    if(voutLimit){
        ...
        return ResultExecute{ex, CreditTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs()), refund.vout.empty() ? CTransaction() : CTransaction(refund)};
    }else{
        return ResultExecute{res, CreditTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs()), tx ? *tx : CTransaction()};
    }
}

std::unordered_map<dev::Address, Vin> CreditState::vins() const // temp
{
...
void CreditState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* CreditState::vin(dev::Address const& _a) const
{
    return const_cast<CreditState*>(this)->vin(_a);
}

Vin* CreditState::vin(dev::Address const& _addr)
{
...
// void CreditState::commit(CommitBehaviour _commitBehaviour)
// {
...
void CreditState::kill(dev::Address _addr)
{
...
void CreditState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void CreditState::deleteAccounts(std::set<dev::Address>& addrs){
...
void CreditState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void CreditState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void CreditState::validateTransfersWithChangeLog(){
...
void CreditState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!CreditState::addressInUse(delegationsAddress)){
        CreditState::createContract(delegationsAddress);
        CreditState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)}, CreditState::version(delegationsAddress));
        commit(CommitBehaviour::RemoveEmptyAccounts);
        db().commit();
    }
}
