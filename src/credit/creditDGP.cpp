...
void CreditDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool CreditDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
    const Consensus::Params& consensusParams = Params().GetConsensus();

    if(defaultData.size() == 39 && (checkData.size() == 39 || (checkData.size() == 40 && blockHeight >= consensusParams.CIP7Height))) {
...
dev::eth::EVMSchedule CreditDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t CreditDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t CreditDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t CreditDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t CreditDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool CreditDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void CreditDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void CreditDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void CreditDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void CreditDGP::createParamsInstance(){
...
dev::Address CreditDGP::getAddressForBlock(unsigned int blockHeight){
...
void CreditDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void CreditDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void CreditDGP::parseStorageOneUint64(uint64_t& value){
...
void CreditDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule CreditDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
