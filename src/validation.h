...
#include <credit/storageresults.h>


extern std::unique_ptr<CreditState> globalState;
...
using ExtractCreditTX = std::pair<std::vector<CreditTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const unsigned int MAX_HEADERS_RESULTS = 10; //limit to COINBASE_MATURITY-1
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // credit
...
class CreditTxConverter{

public:

    CreditTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL, unsigned int flags = SCRIPT_EXEC_BYTE_CODE) : txBit(tx), view(v), blockTransactions(blockTxs), sender(false), nFlags(flags){}

    bool extractionCreditTransactions(ExtractCreditTX& creditTx);
...
private:
    ...
    CreditTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<CreditTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}
