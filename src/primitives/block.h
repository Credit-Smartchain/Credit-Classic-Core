...
public:
    ...
    uint32_t nBits;
    uint32_t nNonce;
    uint256 hashStateRoot; // credit
    uint256 hashUTXORoot; // credit
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        READWRITE(nBits);
        READWRITE(nNonce);
        READWRITE(hashStateRoot); // credit
        READWRITE(hashUTXORoot); // credit
    ...
    void SetNull()
    {
        ...
        nBits = 0;
        nNonce = 0;
        hashStateRoot.SetNull(); // credit
        hashUTXORoot.SetNull(); // credit
    ...
    virtual bool IsProofOfStake() const //credit
    {
    ...
    CBlockHeader& operator=(const CBlockHeader& other) //credit
    {
        if (this != &other)
        {
...
public:
    ...
    std::pair<COutPoint, unsigned int> GetProofOfStake() const //credit
    {
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits          = nBits;
        block.nNonce         = nNonce;
        block.hashStateRoot  = hashStateRoot; // credit
        block.hashUTXORoot   = hashUTXORoot; // credit
