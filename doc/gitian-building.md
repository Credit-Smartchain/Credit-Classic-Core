Gitian building
================
*Setup instructions for a Gitian build of Credit Core using a Debian VM or physical system.*
...
- [Building Credit Core](#building-credit-core)
...
Clone the git repositories for credit and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/The-Terra-Foundation/Credit-Core --recursive
git clone https://github.com/The-Terra-Foundation/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/credit/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the credit repository under 'Fetch and create inputs' to install sources which require
...
To build Credit Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the credit repository.
...
For example:
```bash
URL=https://github.com/laanwj/credit.git
...
# if wanting to use a different version of eth-cpp-credit:
./bin/gbuild --commit credit=${COMMIT},cpp-eth-credit=${ETHCOMMIT} --url credit=${URL},cpp-eth-credit=${ETHURL} ../credit/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the credit git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../credit/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/The-Terra-Foundation/Credit-Core-detached-sigs.git

BTCPATH=/some/root/path/credit
SIGPATH=/some/root/path/credit-detached-sigs

./bin/gbuild --url credit=${BTCPATH},signature=${SIGPATH} ../credit/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/credit-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/credit-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/credit-osx-build.assert
```
...
[creditproject/gitian.sigs](https://github.com/The-Terra-Foundation/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@terra-credit.com) and he will commit them.
